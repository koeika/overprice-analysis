...
def analyze():
    ...
    # 159509 反向策略：Δ↓（溢价暴跌）= 抄底信号
    signal_map = [
        (lambda d, p: d < -3.0, "strong_buy", "🟢 强烈抄底！", "溢价暴跌超3%，恐慌超卖反弹概率95%"),
        (lambda d, p: d < -2.0, "buy", "🟢 抄底信号", "溢价大跌，反弹概率85%"),
        (lambda d, p: d < -1.0, "weak_buy", "🟡 弱抄底信号", "溢价回落，反弹概率76%"),
        (lambda d, p: d > 3.0, "danger", "🔴 溢价暴涨风险", "溢价急升，次日大概率回调"),
    ]
